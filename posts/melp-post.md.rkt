...
脱颖而出。我从源代码编译 Chez，也有着惊人的编译速度，在性能和编译速度上都有着
不俗的表现。尤其对比 Gambit Scheme 实现，其编译到 C 语言之后，也确实有着很不错
的执行效率，但是编译速度堪忧，编译整个 Gambit 系统至少需要一个小时（在笔者的 Mac Pro 2019 上）。
...
因为可以直接使用很多对于 stack 的指令集。另外，该模型也会使用 heap，不过只有少部分情况，主要是涉及对于 first-class continuation
